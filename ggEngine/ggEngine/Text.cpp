...
void ggEngine::Text::Draw(Matrix translatedWorldMatrix)
{
	...
	if (spriteHandle->Begin(D3DXSPRITE_ALPHABLEND) == D3D_OK)
	{
		...
		style.backgroundColor = style.backgroundColor & (opacity << 24 | 0x00FFFFFF);
		style.fontColor = style.fontColor & (opacity << 24 | 0x00FFFFFF);
...
void ggEngine::Text::Transform(Matrix translatedWorldMatrix, LPD3DXSPRITE spriteHandle)
{
	...
	mat *= Matrix::CreateTranslateMatrix(worldPosition.x - width*anchor.x, worldPosition.y - height*anchor.y);
